...
public class S3FileIO implements CredentialSupplier, DelegateFileIO {
  ...
  public S3FileIO(SerializableSupplier<S3AsyncClient> s3) {
    this(s3, new S3FileIOProperties());
  }

  public S3FileIO(SerializableSupplier<S3AsyncClient> s3, S3FileIOProperties s3FileIOProperties) {
    this.s3 = s3;
  ...
  public S3AsyncClient client() {
    if (client == null) {
      synchronized (this) {
        if (client == null) {
          client = S3AsyncClient.crtCreate();
        }
      }
    }
    return client;
