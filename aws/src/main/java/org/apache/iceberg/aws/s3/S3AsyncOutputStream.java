...
public class S3AsyncOutputStream extends PositionOutputStream {
  ...
  S3AsyncOutputStream(
    ...
    this.writeBytes = metrics.counter(FileIOMetricsContext.WRITE_BYTES, MetricsContext.Unit.BYTES);
  ...
  private void uploadParts() {
    ...
    stagingFiles.stream()
        // do not upload the file currently being writted
        ...
        .forEach(
            fileAndDigest -> {
              ...
              CompletableFuture<CompletedPart> future =
                  s3.uploadPart(uploadRequest, AsyncRequestBody.fromFile(file))
                      .handleAsync(
                          (result, thrown) -> {
                            try {
                            ...
                            if (thrown != null) {
                              ...
                              // CompletionException when we will join completable futures.
  ...
  private void completeMultiPartUpload() {
    ...
    // TODO: confirm if this translation is correct
    try {
      s3.completeMultipartUpload(request).join();
    } catch (Throwable t) {
      LOG.error("Failed to complete multipart upload request: ", t);
      abortUpload();
    }
  }
  ...
  private void cleanUpStagingFiles() {
    Tasks.foreach(stagingFiles.stream().map(FileAndDigest::file))
        ...
        .onFailure((file, thrown) -> LOG.warn("Failed to delete stating file: {}", file, thrown))
        .run(File::delete);
  }
  ...
  private void completeUploads() {
    if (multipartUploadId == null) {
      ...
      s3.putObject(
              ...
              AsyncRequestBody.fromInputStream(bufferedInputStream, contentLength, executorService))
          .join();
    } else {
      uploadParts();
